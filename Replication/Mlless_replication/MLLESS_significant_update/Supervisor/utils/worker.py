...
def test(epoch, net, testloader, criterion, device):
    ...
    log_print("Epoch: {}, Test Loss: {:.4f}, Accuracy: {:.2f}%".format(epoch, avg_loss, acc))
...
#)
    ...
    for minibatch in iterator:
        ...
        log_print("W{} fetch dataset {} s".format(worker_id, t_fetch_minibatch))
        ...
        log_print("Step: {} W{} compute grads cost {} s".format(batch_idx, worker_id, gradients_time))
        ...
        log_print("Step: {} W{} listen until receive expected update cost {} s".format(batch_idx, worker_id, time.time() - t_up_0))
        ...
        log_print("Step: {} W{} aggregation cost {} s".format(batch_idx, worker_id, aggregation_time))
        log_print("Step: {} W{} synchronisation cost {} s".format(batch_idx, worker_id, time.time() - t_up_0))
        log_print("Step: {} W{} model update cost {} s".format(batch_idx, worker_id, tmp_model_update))
        ...
        log_print('Worker: {}, Epoch: {}, Step: {}, Time: {}, Loss: {:.6f}, Loss data: {:.6f}, Accuracy: {:.2f}%'.format(worker_id, epoch + 1, batch_idx + 1, time.time() - step_total_time, train_loss / (batch_idx + 1), loss.data, 100. * correct / total))
    ...
    log_print("total_time".format(total_time,file_path))
