...
import pickle
import numpy as np
#from .predictor.scheduler import WorkerScheduler
    ...
    def listen_results(self, n_workers, storage):
        ...
        def receive_results(ch, _method, _properties, body):
            ...
            if storage is not None:
                ...
                #print("i bring model from database")
            ...
            def on_message(ch, _method, _properties, body):
                ...
                if msg_type == 'I':
                    ...
                    if worker_states[w_id].alive:
                        count = step_loss_history.update_loss_history(w_step, loss, 1024)
                        ...
                        if max_time is None or (time.time() - init_time) < max_time:
                            if step_loss_history.get_loss(step) <= threshold:
                                ...
                                if under_threshold == 10:
                                    ...
                                    # Simple scaling logic: reduce workers by a fixed number or percentage if conditions are met
                                    # This is a simplification and may need adjustment for your specific use case
                                    if step % remove_interval == 0 and n_workers > 1:
                                        n_workers -= 1  # Reduce the number of workers by 1
                                        steps_per_epoch = int(n_minibatches / n_workers) + (n_minibatches % n_workers != 0)
                            else:
                                under_threshold = 0
...
class RabbitMQCommunicator(CommonCommunicator):
    ...
    def aggregate_updates(self, storage, model,file_path, _=False):
        start_aggregation = time.time()
        total_updates = len(self.updates_queue)
        log_filename = "log_8_queue_workers.txt"
        ...
        with open(log_filename, "a") as log_file:
            log_file.write("I am worker {}. this is my updated queue {}\n".format(self.worker_id, self.updates_queue))
        ...
        for update_key in self.updates_queue:
            ...
            if aggregated_update is None:
                ...
                aggregated_update = [a + b for a, b in zip(aggregated_update, update)]

        # Calculate the average
        averaged_update = [update / total_updates for update in aggregated_update]

        
        
        aggregation_time = time.time() - start_aggregation
        #pure_aggregation = time.time() - start_aggregation 
        self.receive_end_time = time.time()  # Stop the timer
        total_time = self.receive_end_time - self.send_start_time
        #print("I am worker {}. Total time from sending to receiving all updates: {} seconds".format(self.worker_id, total_time))
        # Write log to file
        
        #with open(log_filename, "a") as log_file:
        #    log_file.write("I am worker {}. Total time from sending to receiving all updates: {} seconds\n".format(self.worker_id, total_time))

        # Update the model with the averaged update
        start_update = time.time()
        model.apply_update(averaged_update)
        tmp_model_update = time.time() - start_update
        
        self.updates_queue = []
        return aggregation_time, tmp_model_update


    def listen(self, n_workers, step, update_available, _):
        #print("start listening")
        self.update_receive_count = 0  # Initialize counter for received updates
...
class BackendRabbitMQCommunicator(RabbitMQCommunicator):
    ...
    def send_updates(self, step, updates):
        self.send_start_time = time.time()  # Start the timer
        ...
        #print("send gradients from communication")
        self.channel.basic_publish(exchange=self.exchange,
                                   routing_key='',
